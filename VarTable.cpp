...
uint64_t VarTable::HashFunc(const std::string& _name) const{
    uint64_t chc = CityHash64(_name.c_str(), _name.length());
    uint64_t hc = chc % this->width;
    return hc;
}
...
bool VarTable::Contains(const std::string& _name) const{
    if(this->buckets){
        uint64_t hc = this->HashFunc(_name);
